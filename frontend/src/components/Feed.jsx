...
export default function Feed({
  ...
  const [likesMine, setLikesMine] = useState([]); // likes by me
  const [likesAll, setLikesAll] = useState([]);   // all likes for counts

  const [q, setQ] = useState("");                 // search box
  const [ordering, setOrdering] = useState("-created_at");

  const [commentTextByPost, setCommentTextByPost] = useState({}); // local inputs
  ...
  async function toggleLike(postId) {
    // optimistic toggle is possible, but keep it simple & correct
  ...
  return (
    <div className="space-y-6">
      <Section
        title="Feed"
        subtitle="Latest writing with likes and discussion."
        right={
          <div className="flex gap-2">
            <input
              ...
              className="rounded-lg bg-white/10 px-3 py-2 text-white placeholder-white/50 outline-none ring-1 ring-white/10 focus:ring-2 focus:ring-white/30"
            />
            <select
              value={ordering}
      ...
      {loading && <Card className="text-white/70">Loading feed…</Card>}
          ...
          return (
            <Card key={post.id}>
              <header className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {Array.isArray(post.tags) && post.tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {post.tags.map((t, i) => (
                        <span key={i} className="text-[11px] rounded-full bg-white/10 px-2 py-0.5 ring-1 ring-white/10">
                          {t}
                        </span>
                      ))}
                    </div>
                ...
                {/* Writer extras on their own posts */}
                {writerExtras && isMine && (onEditPost || onDeletePost) && (
                  <div className="shrink-0 flex gap-2">
                    {onEditPost && (
                      <button
                        onClick={() => onEditPost(post)}
                        className="rounded-lg bg-white/10 px-3 py-1 ring-1 ring-white/10 hover:bg-white/15 flex items-center gap-1"
                        title="Edit"
                      >
                        <FaEdit /> Edit
                      </button>
                    ...
                    {onDeletePost && (
                      <button
                        ...
                        className="rounded-lg bg-red-500/90 px-3 py-1 hover:bg-red-500 text-white flex items-center gap-1"
                        title="Delete"
                      >
                        <FaTrash /> Delete
                      </button>
              ...
              <div className="mt-3 flex items-center justify-between text-sm">
                <button
                  ...
                  className={`rounded-full px-3 py-1 ring-1 flex items-center gap-2 ${
                    liked
              ...
              <ul className="mt-3 space-y-2 text-sm">
                ...
                {comments.length > 4 && (
                  <li className="text-xs text-white/60">…and {comments.length - 4} more</li>
                )}
                {comments.length === 0 && (
      ...
      {!loading && visiblePosts.length === 0 && (
        <Card className="text-white/60">No posts to show.</Card>
      )}
    </div>
  );
}
